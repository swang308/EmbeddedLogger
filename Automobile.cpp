...
void Automobile::addFuel(double _liters)
{
	...
	//Buffer enlarged to 128 and sprintf replaced with snprintf: the formatted
	//text reaches 73 bytes for the longest car ("black 2016 Cadillac Escalade"),
	//which overflowed the original 64-byte buffer and aborted with
	//"stack smashing detected".
	char message[128];
	snprintf(message, sizeof(message), "The %s %d %s %s is full of gas. Discarding the rest...\n", colour.c_str(), year, make.c_str(), model.c_str());
...
void Automobile::drive(double _distance)
{
    ...
    if(fuelInTank < 0) {
        fuelInTank = 0;
	//Same enlargement as addFuel() above, for the same reason.
	char message[128];
	snprintf(message, sizeof(message), "The %s %d %s %s has no gas left in the tank\n", colour.c_str(), year, make.c_str(), model.c_str());
